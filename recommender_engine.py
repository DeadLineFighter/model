...
class RecommenderEngine:
    def __init__(self):
        ...
        google_poi = pd.DataFrame(list(mycol.find({},{"label_types":1,"rating":1,"postcode":1})))

        mycol = mydb["OSM_Transportation"]
        traffic_data = pd.DataFrame(list(mycol.find({},{"Postcode":1})))
    ...
    def data_process(self,data2,df,data_OSM):
        ...
        df16 = return_df(df,c2,"amusement park")
        df17 = return_df(df,c2,"rv park")
        df18 = return_df(df,c2,"park")
        ...
        df6 = return_df(df,c6,"convenience store")
        df7 = return_df(df,c7,"department store")
        df8 = return_df(df,c9,"shopping mall")
        df9 = return_df(df,c10,"supermarket")
        ...
        df10 = return_df(df,c11,"dentist")
        df11 = return_df(df,c12,"doctor")
        df12 = return_df(df,c13,"drugstore")
        df13 = return_df(df,c14,"hospitals")
        df14 = return_df(df,c15,"pharmacy")
        df15 = return_df(df,c16,"physiotherapist")
        ...
        df_Pet = return_df(df,c17,"veterinary care")

        #traffic
        traffic = data_OSM.groupby('Postcode',as_index=False).count()
        traffic.rename(columns={"Postcode":'postcode',"_id":"traffic"}, inplace=True)

        return self.school,self.df_Pet,self.df_park,self.df_diet,self.df_Medical,self.df_shopping,self.traffic
    ...
    def get_recommendations(self,input_data):
        ...
        final_dfs= [postcode_data,self.school,self.df_Pet,self.df_park,self.df_diet,self.df_Medical,self.df_shopping,self.traffic]
        full_data = reduce(lambda left,right: pd.merge(left,right,on='postcode',how="left"), final_dfs) 
        full_data=full_data.fillna(0)
        final_df = full_data[["postcode"]]
